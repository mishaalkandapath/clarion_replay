...
class DQN(nn.Module):
    def __init__(self, n_observations=48, n_actions=52):
...
def optimize_model(use_memory=True, # during trial -- no replay buffer, during rest in bw sessions - yes 
    ...
    if use_memory:
        ...
        assert state is not None and action is not None and reward is not None and next_state is not None
        ...
        non_final_next_states = torch.tensor(next_state, device=device).unsqueeze(0)
