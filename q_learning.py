...
def external_mlp_handle(state_keys, action_keys):
    ...
    def optimize_model(use_memory=True, # during trial -- no replay buffer, during rest in bw sessions - yes 
        ...
        if use_memory:
            ...
            state_batch = torch.tensor(state, device=device).unsqueeze(0)
            action_batch = torch.tensor(action, device=device).unsqueeze(0)
            reward_batch = torch.tensor([reward], device=device).unsqueeze(0)
            non_final_mask = torch.tensor([next_state is not None], device=device, dtype=torch.bool).unsqueeze(0)
            non_final_next_states = torch.tensor(next_state, device=device).unsqueeze(0) if next_state is not None else torch.tensor([], device=device).unsqueeze(0)
