...
from utils import (numpify_grid,
                   shape_shape_rel,
                   shape_start,
                   query_rel_pattern,
                   shape_dict,
                   acc)
from knowledge_init import (Numbers,
                            BrickResponseTask,
...
def present_stimulus(d:BrickConstructionTask, stim_grid: np.ndarray,
    ...
    for i, brick in enumerate(stim_bricks):
        ...
        if not in_send_val:
            ...
            mlp_send_val = (+ mlp_space_1[brick_row_map[brick][1]] ** mlp_space_2[f"n{row_indices[0]}"]
...
def load_trial(construction_space: BrickConstructionTask | BrickConstructionTaskAbstractParticipant, 
    ...
    query_map = {1: response_space.query_rel.left, 2: response_space.query_rel.above,
                 3: response_space.query_rel.right, 4: response_space.query_rel.below}
    ...
    print("Stimulus grid: \n", stim_grid)
    if t_type == "test":
        chunk_test = (+ response_space.io.query_relation ** query_map[trial["Q_Relation"]] 
                      + response_space.io.query_block_reference ** brick_map[trial["Q_Brick_Left"]]
                      + response_space.io.query_block ** brick_map[trial["Q_Brick_Middle"]])
        choice_is_yes = (trial[query_col_map[trial["Q_Relation"]]] == brick_map[trial["Q_Brick_Left"]] 
            and trial[query_col_map[trial["Q_Relation"] - 2 if trial["Q_Relation"] > 2 else trial["Q_Relation"]+2]] == brick_map[trial["Q_Brick_Middle"]])
        ...
        chunk_test = (+ response_space.io.query_relation ** query_map[relation[0]] 
        ...
        if blocks[0] in stim_grid and blocks[1] in stim_grid:
            (_, brick_conn) = brick_connectedness(
                        np.where((stim_grid == blocks[0]) | (stim_grid == blocks[1]), stim_grid, 0)
            )
            choice_is_yes = (brick_conn[relation[0] - 1] == blocks[0] 
                             and brick_conn[relation[0] - 3 if relation[0] in (3, 4) else relation[0] + 1]
                             == blocks[1])
        else:
            choice_is_yes = False
    else: chunk_test = ()
...
def run_participant_session(participant: BaseParticipant, session_df: pd.DataFrame, 
                            session_type="train", q_type="query", init_rules=True):
    ...
    while participant.system.queue:
        ...
        if event.source == participant.start_construct_trial:
            ...
            (grid_stimulus_np, 
             ...
             grid_stimulus_mlp, 
             test_query, 
             choice_is_yes) = load_trial(participant.construction_space, participant.response_space,
            ...
            if participant.past_chosen_goals:
                ...
                if re.match(shape_shape_rel, chosen_goal):
                    ...
                    viz.update_status(chosen_goal.group(3), chosen_goal.group(1), chosen_goal.group(2),
                                      ("TBD" 
                                       if (not len(participant.transition_store) 
                                           or type(participant.transition_store[-1]) is not float) 
                                       else participant.transition_store[-1])) 
                else:
                    chosen_goal = re.match(shape_start, chosen_goal)
                    viz.update_status("start", "", chosen_goal.group(1),
                                      ("TBD" 
                                       if (not len(participant.transition_store)
                                           or type(participant.transition_store[-1])) 
            ...
            simple_plotting(participant.construction_net_training_results,
                            "Steps", "Loss", "figures/construction_net_training.png", viz.fig.number)
            ...
            results.append(((event.time - last_end_construction_time).total_seconds(), 
                            (participant.response_choice.poll()[~participant.response_space.io.output * ~participant.response_space.response] == ~participant.response_space.io.output * ~participant.response_space.response.yes) == choice_is_yes))
            last_end_construction_time = None
    ...
    return (results, construction_correctness,
             all_rule_history, all_rule_lhs_history, all_constructions, all_goal_choices)
...
def run_experiment(num_train_sessions=100, num_test_sessions=20, run_train_only=False):
    ...
    if num_train_sessions:
        ...
        # ---- Plotting ---- 
        simple_snsplot(train_results_df,
                       "trial #", 
                       "construction_correctness", 
    ...
    simple_snsplot(test_results_df,
                   "trial #", 
                   "construction_correctness", 
    ...
    n_sequences = simple_sequenceness(test_rule_choices, test_rule_lhs_information, test_grids)
    plot_sequences(n_sequences)
    goal_sequences = simple_goal_sequencessness(test_goal_choices, test_grids)
